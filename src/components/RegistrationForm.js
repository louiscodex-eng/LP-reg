...
const RegistrationForm = ({onSuccess, isAdminMode, initialData, isEdit }) => {
  ...
  // Passport is only STRICTLY required for new registrations
  ...
  // --- 2. UPDATE LOGIC (JSON) ---
  const updatePayload = {
    // This is the most important field for the update to work
    regID: initialData?.regID, 
    
    // Match the camelCase keys from your successful Postman test
    firstName: firstName,
    ...
    // Include the existing passportUrl so the backend doesn't overwrite it with null
  ...
  response = await fetch("http://84.247.165.61/LabourParty/api/Users/update", {
    ...
    if (response.ok) {
      ...
      // If editing, we just close the modal. If registering, we pass data for the ID card.
      if (isEdit) {
        onSuccess(); // Triggers handleSearch() in AdminDashboard to refresh table
      } else {
        onSuccess(data.data); // Triggers ID Card display
      }
    } else {
    ...
    <form onSubmit={handleSubmit} className="p-2">
      ...
      <div className="row g-3 mb-3">
        <div className="col-md-6">
          ...
          <input className="form-control" placeholder="11 digits" value={phone} onChange={(e) => handleNumericInput(e.target.value, setPhone)} required />
        </div>
        <div className="col-md-6">
          <label className="form-label small fw-bold">NIN</label>
          <input className="form-control" placeholder="11 digits" value={nin} onChange={(e) => handleNumericInput(e.target.value, setNin)} required />
        </div>
      </div>
      ...
      <div className="row g-3 mb-3 border-top pt-3">
        ...
        <div className="col-md-6">
          <label className="form-label small fw-bold">Have Voter's Card?</label>
      ...
      {isVoters === "Yes" && (
        <div className="col-md-12 mb-3">
          <label className="form-label small fw-bold">Voter's Card Number</label>
          <input className="form-control" value={votersCardNo} onChange={(e) => handleNumericInput(e.target.value, setVotersCardNo)} required />
        </div>
      )}
      ...
      <div className="mb-3">
        <label className="form-label small fw-bold">Passport Photo</label>
