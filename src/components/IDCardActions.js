...
const IDCardActions = ({ user }) => {
    ...
    doc.write(`
      <html>
        <head>
          ...
          <style>
            @page { size: auto; margin: 10mm; }
            body { background: white; padding: 20px; display: flex; justify-content: center; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            #id-card-wrapper { width: 450px !important; margin: 0 auto !important; }
            .bg-success { background-color: #198754 !important; -webkit-print-color-adjust: exact !important; }
  ...
  const handleDownloadPDF = async () => {
    // 1. Target the ref directly instead of searching for an ID inside it
    ...
    // 2. Target all text elements for the style fix
    const truncatedElements = element.querySelectorAll('.truncate-text, p, span');
    const originalStyles = [];
  ...
  return (
    <div className="d-inline-flex gap-2">
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: '-9999px', width: '450px' }}>
        <div ref={cardRef} style={{ background: 'white' }}>
          <IDCard user={user} hideControls={true} /> 
        </div>
      </div>
